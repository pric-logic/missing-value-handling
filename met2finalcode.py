...
def replace_missing_value(data):
    """
    Function to replace missing values with attribute mean.
    
    :param d: A numpy array representing the dataset
    :return: A numpy array representing the dataset with missing values replaced by attribute mean
    """
    n, m = data.shape
    for c in range(m):
        am = np.mean(data[:, c])
...
data_matrix_filled = replace_missing_value(data_matrix)
